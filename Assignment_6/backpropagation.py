...
class Model:
    ...
    def __forward_propagate_no_layers(self, row):
        ...
        for label in range(self.total_classes):
            out_layer_values.append(self.__calculate_sigmoid(self.out_weights[label], row[:-1]))
    ...
    def __forward_propagate_1_layers(self, row):
        ...
        layer_one_values = []
        for node in range(self.hidden_nodes):
            out_layer_values.append(self.__calculate_sigmoid(self.layer_one_weights[node], row[:-1]))

        for label in range(self.total_classes):
            out_layer_values.append(self.__calculate_sigmoid(self.out_weights[label], layer_one_values))
    ...
    def __forward_propagate_2_layers(self, row):
        ...
        for node in range(self.hidden_nodes[1]):
            out_layer_values.append(self.__calculate_sigmoid(self.layer_two_weights[node], layer_one_values))

        for label in range(self.total_classes):
            out_layer_values.append(self.__calculate_sigmoid(self.out_weights[label], layer_two_values))
